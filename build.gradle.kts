...
spotless {
    ...
    encoding("UTF-8")
    java {
        // Main Java sources only (license header enforced here)
        target("src/main/java/**/*.java")
        targetExclude("build/**", "**/generated/**")
        // Remove unused imports then order them
        removeUnusedImports()
        trimTrailingWhitespace()
        endWithNewline()
    }
    // Lightweight formatter for test Java sources (no license header, basic cleanup only)
    format("javaTest") {
    ...
    // Future hardening:
    // ratchetFrom("origin/main") // enable once main branch baseline established to only format
    // changed files
}
